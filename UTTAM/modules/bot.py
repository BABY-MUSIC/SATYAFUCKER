...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"•[ 🍹ᴜᴛᴛᴀᴍ тум 🍹 ]•")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"[🍹] 𝐔ᴛᴛᴀᴍ 𝐏ᴀᴘᴀ 𝐈s 𝐇ᴇʀᴇ\n[🏓] 𝐀ʙᴇ 𝐀ʙ 𝐓ᴇʀᴀ 𝐊ʏᴀ 𝐇ᴏɢᴀ\n[⚡] 𝐊ɪsᴋɪ 𝐂ʜᴜᴅᴀɪ 𝐊ᴀʀɴɪ 𝐇ᴀɪ\n\n➜ `{mp} ms`")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"𝐑ᴇʙᴏᴏᴛ 𝐃ᴏɴᴇ\n[🍷] 2 𝐌ɪɴ 𝐖ᴀɪᴛ 𝐏ʟᴇᴀsᴇ\n[🫧] 𝐅ɪʀ 𝐀ᴀʏᴇɢᴀ 𝐓ᴇʀɪ 𝐌ᴀᴀ 𝐂ʜᴏᴅɴᴇ 𝐔ᴛᴛᴀᴍ 𝐑ᴀᴛʜᴏʀᴇ✬ ")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"»🍃 𝐔ᴛᴛᴀᴍ 𝐊ᴀ 𝐄к 𝐎ʀ 𝐍ᴇᴡ 𝐁єтα 𝐀ᴅᴅ 𝐇σ 𝐆уα 🍃")
        target = ""
        ...
        try:
            ...
            await ok.edit("𝐀вє 𝐉ʜᴀᴛ 𝐊є 𝐁ααℓ 𝐔ραʀ 𝐒є 𝐑єᴘℓу 𝐃є 𝐑αнα 𝐇αι 𝐁ααᴘ 𝐊σ")
            return
